...
# Subproblem
class Prosumer:
    def __init__(self, inc, pref, data=None, rho=1):
        self.data = expando()
        self.data.a = data['a']
        self.data.b = data['b']
        ...
        self.data.CM = (data['ntype']=='CM')
        self.data.partners = inc.nonzero()
        self.data.pref = pref[inc.nonzero()]
    ...
    def _update_objective(self):
        augm_lag = (-sum(self.y*( self.variables.t - self.t_average ) ) + 
                    ...
                    # + sum((self.variables.t - self.t_old)*(self.variables.t - self.t_old))
                   )
